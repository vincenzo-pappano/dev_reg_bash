#!/usr/bin/env bash
# dev_reg_get_state.sh — GET /devices/{id} (sourcing-friendly, with diagnostics)
# Source this file:  . ./dev_reg_get_state.sh [device_id]
#
...
code="$(
  curl -sS --http1.1 \
    ...
    "https://${hostname}/${api_version}/devices/${DEVICE_ID}" \
...
if (( QUIET == 0 )); then
  echo "=== BODY ==="
  if jq -e . >/dev/null 2>&1 <"$body"; then jq . <"$body"; else cat "$body"; fi
fi
